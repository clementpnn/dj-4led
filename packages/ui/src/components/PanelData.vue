  ...
  <div class="data-panel">
    ...
    <div class="spectrum-section">
      <div
        ...
        :class="{ streaming: isStreaming, offline: !isConnected }"
      >
          ...
          <div class="spectrum-bars">
            ...
            >
              ...
              <div
                v-if="value > 0.7"
                class="spectrum-glow"
                :style="getGlowStyle(value, index)"
              ></div>
            </div>
          </div>
        ...
        <div class="spectrum-metrics">
          <div class="metric">
            ...
            <div class="metric-bar">
              <div
                class="metric-fill rms"
                :style="{ width: `${rmsValue * 100}%` }"
              ></div>
            </div>
          ...
          <div class="metric">
            ...
            <div class="metric-bar">
              <div
                ...
                :style="{ width: `${maxSpectrumValue * 100}%` }"
              ></div>
            </div>
            <span class="metric-value"
              >{{ (maxSpectrumValue * 100).toFixed(1) }}%</span
            >
          </div>
        </div>
      </div>
...
const maxSpectrumValue = computed(() => {
  if (!processedSpectrumData.value || processedSpectrumData.value.length === 0)
    return 0;
...
const rmsValue = computed(() => {
  if (!processedSpectrumData.value || processedSpectrumData.value.length === 0)
    return 0;
  const sum = processedSpectrumData.value.reduce(
    (acc, val) => acc + val * val,
    0,
  );
...
const getSpectrumBarStyle = (value: number, index: number) => {
  ...
  return {
    ...
    boxShadow:
      value > 0.3
        ? `0 0 ${value * 10}px hsl(${hue}, ${saturation}%, ${lightness}%)`
        : "none",
  ...
  (newSpectrumData) => {
    if (newSpectrumData && newSpectrumData.length > 0) {
      ...
      if (peakValues.value.length !== processedSpectrumData.value.length) {
        peakValues.value = new Array(processedSpectrumData.value.length).fill(
          0,
        );
      }
    }
