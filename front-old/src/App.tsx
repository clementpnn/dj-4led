...
class FrameRenderer {
    ...
    renderFrame(data: number[] | Uint8Array, width: number = 64, height: number = 64) {
        if (!this.ctx || !this.imageData || !this.canvas) return;

        // Convertir en Uint8ClampedArray si nécessaire
        const frameData = data instanceof Uint8ClampedArray ? data : new Uint8ClampedArray(data);
...
export default function App() {
                    ...
                    switch (data.type) {
                        ...
                        case 'compressedframe':
                            if (decompressorRef.current) {
                                ...
                                const decompressed = await decompressorRef.current.decompress(compressed);
                                frameRendererRef.current?.renderFrame(decompressed, data.width, data.height);
                            }
                            break;
                        ...
                        case 'spectrum':
                            ...
                            setSpectrum(prevSpectrum => {
                                ...
                                const isDifferent = data.data.some((v, i) => Math.abs(v - prevSpectrum[i]) > 0.05);
    ...
    const updateCustomColor = useCallback(
        (r: number, g: number, b: number) => {
            if (ws && ws.readyState === WebSocket.OPEN && currentColorMode === 'custom') {
                const rNorm = r / 255;
                const gNorm = g / 255;
                const bNorm = b / 255;
                ws.send(
        ...
        <div className="container">
            ...
            <div className="preview-section">
                <div className="preview-container">
                    <canvas ref={canvasRef} width={64} height={64} className="led-preview" />
            ...
            <div className="effects-section">
                ...
                <div className="effects-grid">
                    {effects.map(effect => (
                        <button
                            key={effect.id}
                            className={`effect-button ${currentEffect === effect.id ? 'active' : ''}`}
            ...
            <div className="color-section">
                ...
                <div className="color-modes">
                    {colorModes.map(mode => (
                        <button
                            key={mode.id}
                            className={`color-mode-button ${currentColorMode === mode.id ? 'active' : ''}`}
